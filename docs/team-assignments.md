...
2. 필수 입력: 날짜, 하드 제약, 개인 목표·절대상한 예산, 가치 정책
...
5. `FactConstraintValidator` 한 카테고리 수직 경로
6. Proxy·Arbiter 토론과 leximin 선택 trace
7. Orchestrator 전역 가드
8. Finalizer 연속성 감사와 `FinalPlanRecord`
...
4. 기준선 A~E 비교와 사용자 단위 데이터 분리
5. 축 제거, 중복성, 순위 안정성, 도시 전이 분석
...
한 도시·한 카테고리에서 실제 공급자 또는 명시적 fixture로 후보 조달부터 차단/계약까지 관찰한다. 전체 제품 완료로 부르지 않는다.
