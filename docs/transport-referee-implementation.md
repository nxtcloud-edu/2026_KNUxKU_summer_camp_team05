...
| 유아차 동반 | `strollers > 0` | 계단 환승 다수 경로 감점(실격 아님) |
...
R-TRANS-4는 그룹 여행의 고전적 불공정 지점이다. 운전자는 여행 내내 일하고 술도 못 마신다. **양보 크레딧을 자동으로 +0.2 부여**하고 후속 라운드에서 우선권을 준다.
...
| 도시간 교통 시간표 | 7d | `origin:dest:date` | 전역 |
| 막차 시각 | 30d | `from:to` | 전역 |
| 패스 정보 | Pack 갱신 시 | `packId:passId` | 전역 |
| 유가·통행료 | 24h | `route` | 전역 |
| 실시간 경로(택시 요금 추정) | 1h | 좌표 해시 | 전역 |
...
휠체어·계단·유아차 접근성, 막차, 심야 안전 경로는 fail-closed다. 경로 제공자가 이를 보장하지 못하면 보행·환승 후보를 채택하지 않고, 검증된 택시·전용 이동 대안과 예산을 함께 제시한다. 우천·장애·막차 상황의 대체 경로도 같은 정책으로 검증한다.
