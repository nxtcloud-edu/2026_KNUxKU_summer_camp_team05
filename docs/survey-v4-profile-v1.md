...
1. 날짜 가용성, 하드 제약, 가치 정책, 개인 목표·상한 예산을 필수 입력으로 받는다.
...
| 선택지 | 사용자 문구 | 갱신 신호 |
| --- | --- | --- |
| `Q03_A` | 이동과 활동을 대부분 모두 함께하고 싶어요 | `C04 +2`, `split_policy=no_split` |
| `Q03_B` | 핵심 일정은 함께하고 일부 자유시간은 괜찮아요 | `C04 0`, `split_policy=limited` |
| `Q03_C` | 반나절 정도는 취향별로 나뉘어도 괜찮아요 | `C04 -2`, `split_policy=allowed` |
...
| Q03 | `C04` | `split_policy` | `together`, `limited_split` |
...
취향 질문은 건너뛰기를 허용하며 `skipped` 원본을 보존한다. 날짜·하드 제약·개인 절대 상한처럼 토론 안전성에 필요한 필수 입력만 제출 차단 조건으로 사용한다. FE는 미응답 취향을 기본 중립값으로 채우지 않는다.
...
| 2 | 꼭 지켜야 할 조건 | 안전·접근성·절대 불가·가치 정책 |
