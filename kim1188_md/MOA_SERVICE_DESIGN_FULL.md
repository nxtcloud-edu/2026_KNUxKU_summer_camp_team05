# MOA 그룹 여행 의사결정 서비스 설계 정리본

> 최초 기준 시점: 2026-08-13 KST  
> 최종 수정: 2026-08-14 KST  
> 문서 목적: 지금까지 논의한 서비스 구조를 제품 관점에서 통합하고, 확정된 방향과 아직 검증되지 않은 가설을 분리한다.  
> 이 문서는 현재 웹 화면이나 팀 저장소의 구조를 그대로 정답으로 선언하는 문서가 아니다.
> 제품 수준 충돌에 대한 최신 결정은 [PRD-MOA-PRODUCT-DECISIONS.md](./PRD-MOA-PRODUCT-DECISIONS.md)를 우선한다.
> 에이전트 명칭·계층·권한은 [ADR-MOA-AGENT-NAMING-HIERARCHY.md](./ADR-MOA-AGENT-NAMING-HIERARCHY.md)를 우선한다.
...
- **[미결정] MVP 도시 목록**: 실제 데이터·API·사용 권리를 비교한 뒤 한국 2개와 일본 2개를 확정
...
| 목적지 유형 | 우선 활성화 가능성이 높은 축 |
| --- | --- |
| 서울·도쿄 | 전시, 트렌드·팝컬처, 쇼핑, 야간 활동, 혼잡, 환승 |
| 부산·오사카·후쿠오카 | 현지 음식, 시장, 해산물, 웨이팅, 야경, 지역 집중도 |
| 제주·강릉·여수·삿포로 | 자연, 실외, 날씨 영향, 이동수단, 해산물, 동선 폭 |
| 교토 | 역사·전통, 대표 명소·로컬 탐색, 혼잡, 보행, 이른 시작, 현지형 숙박 |
...
| 한 목적지·카테고리에서 활성화할 축 | 대체로 4~6개 |
| p차원 후보 풀 | 내부적으로 약 `3p~5p`; 사용자에게 전부 노출하지 않음 |
| 한 화면/카테고리 최초 후보 카드 | 대체로 4~8개 |
| 카테고리별 적응형 추가 질문 | 0~2개 |
| 일정 후보 | 약 3개 + 필요 시 1개 질문 |
...
초기 온보딩에 몰아넣지 않고 계획 과정 전체에 분산하는 경우의 카테고리별 노출 가설은 다음과 같다.

| 카테고리 | 최초 노출 가설 | 추가 질문 | 일반적 총 노출 |
| --- | --- | --- | --- |
| 여행 헌장·현황 | 빠른 선택 약 6개 | 원칙상 없음 또는 누락 제약만 | 약 6개 |
| 오는 길·가는 길 | 시나리오 약 4개 | 0~2개 | 4~6개 |
| 숙소 | 후보 카드 약 6개 | 0~2개 | 6~8개 |
| 갈 곳·할 일 | 후보 카드 6~8개 | 0~2개 | 6~10개 |
| 식사 | 후보 카드 6~8개 | 0~2개 | 6~10개 |
| 일정·현지 이동 | 일정안 약 3개 | 0~1개 | 3~4개 |
| 통합 확정 | 취향 질문 없음 | 사실 재확인만 | 0개 |

이 표의 “노출”은 질문 수와 동일하지 않다. 한 화면에서 여러 후보를 비교할 수 있고, 최초 프로필 상호작용 10~12개라는 가설과 중복 계산해서는 안 된다.
...
- 최초 프로필 형성: 보통 10~12개 상호작용, 최대 약 14개
- 같은 여행·같은 목적지 재논의: 결과 공개 뒤 불만 카테고리 중심 기본 2개, 최대 4개
- 같은 국가의 새 목적지: 목적지 특화 4~6개
- 한국↔일본처럼 국가 전환: 기본 6개, 불확실하면 최대 8개

이 숫자는 화면 수, 카드 수, 응답 수 중 무엇을 세는지 먼저 정의한 뒤 사용자 실험으로 조정해야 한다.

### 8.5 중단 조건

- 미확인 하드 제약이 없음
- 상위 후보가 현재 불확실성에서도 안정적임
...
**[결정]** Python 중심의 결정론적 코드로 가격, 통화·세금, 주소·place ID·좌표, 정확한 예약 날짜·인원·객실, 영업시간, 이동 연결, 페이스, 개인별 예산, 근거 만료를 검사한다.
...
**[MVP 가설]** 갈등이 해결되지 않을 때 활동·식사·일부 현지 이동에서만 일시 분리를 허용한다. 오는 길·가는 길과 거점 숙소는 기본적으로 공동 자원으로 취급한다. 원치 않는 사용자를 점수 때문에 혼자 분리하지 않으며, 영향받는 당사자의 동의를 필요로 한다. 자동 채택은 0단계에서 사용자가 미리 위임한 인원·시간·비용·안전 범위 안에서만 가능하다. 이를 넘는 구체 분리안은 대리인이 동의를 추정하지 않고 `NO_SAFE_DECISION / HOLD`로 6단계 부분 결과에 넘긴다. `SplitPlan`에는 다음을 기록한다.
...
- `provider`
- `tool_or_endpoint`
- 정규화한 요청 인자: 여행 날짜·시간대, 인원·객실 조합 또는 예약 슬롯, 통화
- 원본 응답 참조와 해시
- 반환된 사실과 단위
- 정규화 주소, 위도·경도, 행정구역
- `observed_at`
...
> **[구현 관찰] 기준 커밋:** `70215bf0379531ac936b2635b3970afd300ae8ed`, 2026-08-13 07:48:26 UTC

현재 저장소에는 프론트엔드, API·Worker 골격, PostgreSQL 리포지토리, 공용 계약, Planning Graph, 카테고리 라운드, 이의 제기·재실행 파이프라인이 존재한다. 그러나 README는 저장소를 **설계 문서 단계**로 명시하고, 저장소의 옛 명칭 기준 DateResolver·Data Agent·실제 심판·Supervisor·여행 API 통합은 미착수라고 표시한다. [현재 README](https://github.com/nxtcloud-edu/2026_KNUxKU_summer_camp_team05/blob/70215bf0379531ac936b2635b3970afd300ae8ed/README.md)

현재 코드에서 관찰되는 사항:

- 설문 v2 상태는 가용 날짜, 하드 제약, 여행 스타일 점수, 활동 점수, `mustDo`, `avoid`를 가진다. v3의 5·3·1 중요도 초안도 병존한다. 제품 결정은 **사용자 표시 5·3·1 + 축 기반 장기 프로필·구체 태그**이며, 저장소에는 아직 이 변환 계약이 구현되지 않았다. [현재 설문 상태](https://github.com/nxtcloud-edu/2026_KNUxKU_summer_camp_team05/blob/70215bf0379531ac936b2635b3970afd300ae8ed/apps/web/src/formState.ts) [v3 초안](https://github.com/nxtcloud-edu/2026_KNUxKU_summer_camp_team05/blob/70215bf0379531ac936b2635b3970afd300ae8ed/packages/contracts/src/preference-v3.ts)
- Planning Graph는 날짜, 항공, 이동 정책, 거점, 숙소, 활동, 식사, 일정, 예산, 예약 준비, 검증, 문서 노드를 둔다. 저장소의 `STALE` 방향은 목표와 맞지만, 제품 계약에서는 별도 상태 계층으로 세지 않고 `isStale/staleReason/sourceVersion` 메타데이터로 정규화한다. [현재 계획 계약](https://github.com/nxtcloud-edu/2026_KNUxKU_summer_camp_team05/blob/70215bf0379531ac936b2635b3970afd300ae8ed/packages/contracts/src/planning.ts)
- 현재 라운드는 프레이밍·항공·교통·숙소·활동·식사·일정·예산으로 정의되어 있다. 수치 임계값과 턴 상한은 저장소 가설이며 사용자 연구로 확정된 정책이 아니다. [현재 라운드 계약](https://github.com/nxtcloud-edu/2026_KNUxKU_summer_camp_team05/blob/70215bf0379531ac936b2635b3970afd300ae8ed/packages/contracts/src/rounds.ts)
- 저장소는 한국 5곳·일본 6곳을 목표로 적고 있지만 제품 MVP 결정은 **한국 2곳·일본 2곳**이다. 저장소의 Pack 목록을 이후 줄여야 한다.
...
- 정확한 최초 질문 수와 화면 수
- 자유서술, 카드 비교, 슬라이더, 직접 수정 중 어떤 조합을 쓸지
- 그룹 인원 범위
- 한국 2개·일본 2개 Destination Pack의 실제 도시 목록
- 사용자 프로필 확인·수정 방식

### 취향 모델

- 47개 v0 축의 실제 독립성·예측력
- 보이는 5·3·1을 내부 축 강도로 보정하는 값과 축별 최종 문구
- 강한 선호와 하드 제약의 UI 경계
- 반복 행동을 장기 프로필로 승격하는 조건
- 목적지별 축 활성화 모델
...
- 한국·일본에서 사용할 실제 POI·교통·가격·숙박·예약 API
- 데이터 사용 권리, 비용, 호출 제한, 캐시 유효기간
...
1. 한국 2개·일본 2개 Pack 후보를 조사하되, 먼저 한국 1개·일본 1개 도시의 실제 후보 데이터로 종단 검증한다.
2. 47개 축 중 후보 분산이 있는 축만 표시한다.
3. 축 중복, 질문 이해도, 한 질문의 정보량을 소규모 인터뷰로 검사한다.
4. 토론 시작 전 4~8개 카드와 0~2개 추가 질문으로 후보 순위를 얼마나 안정적으로 예측하는지 측정한다.
5. 사용자별 Proxy가 원본·예외·제약을 얼마나 정확히 재현하는지 먼저 평가한다.
6. 한 카테고리에서 `후보 조달 → 대리인 입장 → 감독 → 결정 계약`을 끝까지 검증한다.
7. 실제 경로·시간·영업·예산 검사로 계획 타당성을 확인한다.
8. 재논의 1회가 불만족을 개선하면서 다른 사용자의 제약을 지키는지 테스트한다.
9. 같은 국가의 새 목적지와 국가 전환에서 추가 질문량을 비교한다.

10. 그 후에만 축·질문·도시·에이전트 수를 확대한다.

6번은 해커톤용 세로 절단 검증이지 제품 완결 기준이 아니다. 제품 완결은 0단계 입력, 1~5단계 다섯 실제 계약, 6단계 연속성·통합 검증을 모두 요구하며 fixture 계약으로 이를 대체했다고 주장하지 않는다.
...
| 토론 시작 전 카테고리당 4~8개 카드와 최대 2개 추가 질문이 적정하다 | 낮음~중간 | UX 출발 가설이며 사용자 실험 전 |
...
3. [팀 저장소 README, 고정 커밋](https://github.com/nxtcloud-edu/2026_KNUxKU_summer_camp_team05/blob/70215bf0379531ac936b2635b3970afd300ae8ed/README.md) — 현재 구현·검증 상태
4. [현재 설문 상태 계약](https://github.com/nxtcloud-edu/2026_KNUxKU_summer_camp_team05/blob/70215bf0379531ac936b2635b3970afd300ae8ed/apps/web/src/formState.ts) — 현재 목업 입력 구조
5. [현재 Planning Graph 계약](https://github.com/nxtcloud-edu/2026_KNUxKU_summer_camp_team05/blob/70215bf0379531ac936b2635b3970afd300ae8ed/packages/contracts/src/planning.ts) — 노드·상태·의존성 구현 스냅샷
6. [제품 수준 결정 PRD](./PRD-MOA-PRODUCT-DECISIONS.md) — 이번 재설정의 권위 문서
7. [에이전트 명칭·계층 ADR](./ADR-MOA-AGENT-NAMING-HIERARCHY.md) — 공식 명칭·권한·호출 관계
8. [현재 Round 계약](https://github.com/nxtcloud-edu/2026_KNUxKU_summer_camp_team05/blob/70215bf0379531ac936b2635b3970afd300ae8ed/packages/contracts/src/rounds.ts) — 라운드·임계값 구현 스냅샷
9. [MOA 배포 목업](https://moa-henna.vercel.app) — 현재 화면 흐름 참고용
